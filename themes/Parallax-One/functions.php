...
function parallax_one_wp_page_menu() {
	...
	echo '<li><a href="' . $site_url . '/#issues' . '">Issues</a></li><li><a href="' . $site_url . '/#demands' . '">Demands</a></li><li><a href="' . $site_url . '/#statement' . '">Sign The Statement</a></li><li><a href="' . $site_url . '/#latestnews' . '">News</a></li><li><a href="' . $site_url . '/act/' . '">Take Action</a></li><li><a href="' . $site_url . '/learn/' . '">Learn More</a></li><li><a href="https://fundrazr.com/labradorlandprotectors" target="_blank">Donate</a></li>';
	echo '</ul>';
}
...
function supporter_logos(){
	...
	$logohtml = '<div class="clients white-bg" id="clients" role="region" aria-label="Supporter Logos"><div class="container"><ul class="client-logos">';
	foreach ( $parallax_one_logos_decoded as $parallax_one_logo ) {
		...
		if ( ! empty( $image ) ) {
			if ( ! empty( $link ) ) {
				$logolistitemcontent = '<a href="' . $link . '" title=""><img id="client-logos-img-large" src="' .  parallax_one_make_protocol_relative_url( esc_url( $image ) ) . '" alt="Logo"></a>';
			} else {
				$logolistitemcontent = '<img id="client-logos-img-large" src="' . parallax_one_make_protocol_relative_url( esc_url( $image ) ) . '" alt="Logo">';
			}
			$logolistitem = '<li>' . $logolistitemcontent . '</li>';
		} // End if().
		$logohtml .= $logolistitem;
	} // End foreach().
	$logohtml .= '</ul></div></div>';
	return $logohtml;
}
...
function get_meta_values( $keyone = '', $keytwo = '', $type = 'post', $status = 'publish' ) {
    ...
    $r = $wpdb->get_col( $wpdb->prepare( "
        SELECT CONCAT(pm.meta_value, ', ', pmt.meta_value) FROM {$wpdb->postmeta} pm
        LEFT JOIN {$wpdb->postmeta} pmt on pmt.post_id = pm.post_id
        LEFT JOIN {$wpdb->posts} p ON p.ID = pm.post_id
        WHERE pm.meta_key = '%s'
        AND pmt.meta_key = '%s' 
        AND p.post_status = '%s' 
        AND p.post_type = '%s'
    ", $keyone, $keytwo, $status, $type ) );


    return $r;
}

function submission_names() {
  $sub_names = get_meta_values('_field_6', '_field_11', 'nf_sub');
...
//   register_rest_route( 'wp/v2', '/submission_names', array(
//     'methods' => 'GET',
//     'callback' => 'submission_names',
//   ) );
// } );
...
function sub_names_list( $atts ){
    $sub_names_array = submission_names();
    // $sub_names_list = implode(', ', $sub_names_array);
    return array2ul($sub_names_array);
}

add_shortcode( 'sub_names_list', 'sub_names_list' );
add_shortcode( 'supporter_logos', 'supporter_logos' );
...
wp_enqueue_script( 'docReady', get_theme_file_uri( '/js/docReady.js' ));
wp_enqueue_script( 'helloworld', get_theme_file_uri( '/js/helloworld.js' ), array( 'docReady' ));